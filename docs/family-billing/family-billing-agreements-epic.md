...
| R9 | **RBAC surface** — `billing` registry needs no change, but VicePrincipal held `billing:*` by default (`role-assignment.entity.ts:179`, was an UNVERIFIED seed grant) while agreements introduce negotiated pricing. | ✅ **RESOLVED (Backend Slice 2, owner decision 2026-07-09):** VP narrowed to `billing:view,create,edit` — `billing:manage`/`delete` (agreement activate/override/cancel, payment reversals, gateway config) reserved to Principal + Accountant. Permission-matrix snapshot regenerated; FE abac already omitted VP billing, so no UI mismatch. |
...
- ~~Drain remaining 13 KNOWN_DRIFT routes~~ — **partially done (Backend Slice 1, BH-1.5):** the one true drift, `payments/{paymentId}/reconcile`, is registered + drained (13→12). The remaining 12 are identity/session spec-alignment entries for a dedicated hygiene sprint.
- ~~AGREEMENT_BYPASSED → queryable audit rows, then provenance `overrides[]`~~ — **DONE (Backend Slice 1, BH-1.2/1.3):** persisted as `finance.agreement.bypassed` FinanceAuditService rows keyed by invoiceId+studentId; provenance `overrides[]` implemented.
- ~~AY label→id resolution for discount-rule scoping~~ — **DONE (Backend Slice 1 BH-1.4 + service-auth follow-up):** finance resolves label/UUID→yearId via identity for ALL invoice-generating roles + the webhook. The BH-1.4 first cut worked only for admin roles (Principal/VP/TenantAdmin) because it called identity's operator route `GET /schools/:id/academic-years` (`@RequirePermission('scheduling','view')`) — the Accountant role and the enrollment-webhook context lack that permission, so the call 403'd and AY-scoping degraded to unscoped for them. **Fixed** with a service-authenticated INTERNAL read: identity now exposes `GET /internal/schools/:id/academic-years` gated by `InternalApiKeyGuard` (`x-internal-api-key`, the per-tenant sha256 key already used by the enrollment webhook), returning the SAME rows via the SAME `AcademicYearsService.listAcademicYears` with NO permission gate (tenant isolation still enforced via the forwarded operator JWT → TokenVendingMachine). Finance's `getAcademicYears` retargets to that route + sends the internal key. Result: Accountant + webhook now RESOLVE the year; the 3-state resolver (resolved/not_found/unavailable) semantics are unchanged — only the transport auth changed. Route is Service-Connect-only (route-drift EXEMPT, authz-coverage classifies it `internal`).
- ~~Per-term guard TOCTOU (risk R11)~~ — **DONE (Backend Slice 1, BH-1.1):** conditional `AGREEMENT_TERM_LOCK#{schoolId}#{studentId}#{agreementChainId}` row written `attribute_not_exists` in the same `transactWrite` as the (agreement-priced) invoice, and deleted atomically when the invoice is cancelled/written-off (so the lock agrees with the read-guard's dead-status re-billing path). "Once per term per chain" is now atomic under concurrent generation. Standard invoices keep the bare `putItem` (golden byte-identity preserved).
- ~~R9 VicePrincipal `billing:*` default-grant review~~ — **DONE (Backend Slice 2, 2026-07-09):** narrowed to `billing:view,create,edit`; `manage`/`delete` reserved to Principal + Accountant.
