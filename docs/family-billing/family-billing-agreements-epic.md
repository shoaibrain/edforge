...
- ~~AY label→id resolution for discount-rule scoping~~ — **partially done (Backend Slice 1, BH-1.4):** finance resolves label/UUID→yearId via identity for admin roles (Principal/VP/TenantAdmin). **Still open:** effective only for those roles — Accountant manual generation and the enrollment-webhook path safely degrade to unscoped (non-regressive, WARN-logged) because identity's `GET /schools/:id/academic-years` requires `scheduling:view` and finance has no outbound service-auth path. **Follow-up:** add a service-authenticated (internal-API-key) academic-years read so AY-scoping is effective for all invoice-generating roles + the webhook, independent of operator role. (Owner decision 2026-07-08: ship Slice 1 with the documented degrade; service-auth read is a separate ticket.)
