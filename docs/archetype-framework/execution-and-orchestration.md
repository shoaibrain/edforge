...
| GB1 — country→archetype calendar + regional cluster + country-branch lint | 🟢 | **Code merged on `main`** (GB1.1/1.2a/1.2b/1.4/1.5/1.6; GB1.3 deferred-non-premature, identical output for PABSON-in-NPL). #245 added the country-branch CI lint (`archetype-invariants.yml`). **PROD NOT VERIFIED in-repo:** `docs/deploys/INDEX.md` still carries the GB1 entry as **🔴 prod smoke FAILED 2026-06-04** — PABSON with omitted `calendarSystem` returned `gregorian` (a pre-existing `createSchoolSchema.default('gregorian')` masking bug GB1.1 inherited via the global ZodValidationPipe). Fix coded in 0.68.0 (`.optional()` + regression specs, #243), but INDEX's own "re-roll → re-smoke → flip 🟢" step was never completed: no committed re-smoke log, no June prod logs (newest are 2026-05-26). **Reconcile w/ deploy operator.** |
| GB2 — board-exam/curriculum/exam-pattern seeding | 🟡 | **Partial + prod-unverified.** Merged: GB2.1–2.5 (`getOrSeedBoardExams` idempotent + concurrency-safe, `GET /board-exams` seed-on-empty, archetype curriculum-ref default, `GET /exams/exam-pattern`). **Not built:** GB2.6 re-seed CloudWatch alarm, GB2.9 `seed-missing-board-exams.ts` backfill (both deferred in the runbook). **PROD NOT VERIFIED in-repo:** `prod-gb2-deploy-prompt.md` is an **unexecuted template** (merge SHA = literal placeholder); no GB2 deploy summary / smoke log / INDEX entry. The "7/7" has no committed backing artifact. **Reconcile w/ deploy operator.** |
...
The code is on `main`; the **deploy ladder's mandated evidence is not committed in-repo**. None of this means a deploy *didn't* happen — Mac-side deploys can produce runbooks/logs that were never `git add`-ed — but per the golden-shape ladder, a sprint is only "prod-verified" with a committed runbook + smoke log. Open items to reconcile with the deploy operator:

| Gap | Evidence | Action |
|---|---|---|
| **GB1 prod re-verification** | `INDEX.md` GB1 entry still 🔴 (smoke FAILED 2026-06-04, `gregorian` vs `bikram_sambat`); 0.68.0 fix merged but re-smoke never recorded; no June prod logs | Either commit the green re-smoke log + flip INDEX 🟢, or run the prod re-deploy + `gb1-calendar-derivation.ts` smoke now |
