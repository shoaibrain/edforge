...
| **24** | **`letterheadBackgroundSrc` is declared in pdf-renderer descriptor type but no renderer consumes it** (pre-existing pdf-renderer feature gap from C.0.3 primitives, surfaced by M3 phase 2 prod test). | **P1 feature gap** | ✅ **Closed 2026-05-27 via server PR #211** — `<Page>` extended with optional `letterheadBackgroundSrc` prop rendering an absolutely-positioned `<View fixed>` + `<RpdfImage>` at full page bounds beneath content (Z-order via DOM declaration). Both finance renderers thread `branding.letterheadBackgroundSrc`. New `isPdfLetterheadKey` projection guard drops PDF-keyed letterheads (react-pdf v3.4.5 renders only raster). 4 new render-smoke specs + 7 new isPdfLetterheadKey unit specs. `@aibrains/pdf-renderer@0.7.0` live on npm; finance image `sha256:90359d1c…` + identity image `sha256:43a8bf1f…` running on prod-basic. |
...
| # | Work | Type | Scope estimate | Status |
|---|---|---|---|---|
| 0.6.B.1 | **Issue #24 — `letterheadBackgroundSrc` actually renders.** | Feature gap closure | ~80 LOC pdf-renderer + 4 render-smoke specs + isPdfLetterheadKey helper + 7 unit specs + cross-repo pin bump + ECR rebuild | ✅ **Shipped 2026-05-27 PM** via server PR #211 (`@aibrains/pdf-renderer@0.7.0` live; finance + identity ECR rolled). |
...
Reading the matrix (refreshed 2026-05-27 PM after C.1.8 ship):

1. ✅ **0.6.B.1** (letterhead rendering) — closed via server PR #211.
2. **0.6.A.2** (eager preview) + **0.6.A.1** (M1.5-FU.7.2-.7.5) — small UX cleanup behind shipped surfaces. **IN FLIGHT — next.** Single morning.
3. **0.6.C.1** (M4 templates read) — opens read-only template inspection in Settings, no backend writes needed.
4. **Pause / sync with backend** — confirm C.2.1 backend ETA (currently "NOT shipped" per §0.4 master plan). If C.2.1 not slotted, move to D.4 BLE Workflow per master plan §0.4 critical-path move #2, returning to M5+ once backend lands.
...
| **M3 phase 3** (NEW, scope below) | Letterhead PDF rendering (Issue #24) — ✅ shipped 2026-05-27 via server PR #211. Asset-upload eager preview UX (Issue #25) — 🔲 still open. | Issue #24 closed; Issue #25 pending as part of frontend closeout housekeeping | None | 🟡 **Partial — letterhead renders; eager preview deferred to closeout housekeeping branch** |
