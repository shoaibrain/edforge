...
> **Updated:** 2026-05-27 PM — **EPIC-C C.1 + branding (M2 + M3) + C.1.8 letterhead rendering all 🟢 shipped to prod end-to-end.** C.1 backend stacked deploy shipped 2026-05-26 (commit `d2cf929`). C.1.6 frontend stack (23 PRs across M0/M1/M1.5-FU) shipped 2026-05-26 to Vercel. M2 Branding read (PR #88) + M3 phase 1 (PR #89) shipped 2026-05-26. M3 phase 2 (FE PR #90 + server hotfixes #209 + #210) shipped 2026-05-27 morning. **C.1.8 letterhead rendering** (PR #211 + `@aibrains/pdf-renderer@0.7.0` + finance image `sha256:90359d1c…` + identity image `sha256:43a8bf1f…` ) shipped 2026-05-27 PM — closes the renderer feature gap surfaced by M3 phase 2 prod test (`letterheadBackgroundSrc` declared in descriptor type but no primitive consumed it). Operators can now upload PNG/JPEG letterheads via `/settings/branding` and they render as page backgrounds on the resulting Invoice + Receipt PDFs (PDF letterheads stored but skipped at projection via `isPdfLetterheadKey` guard pending future renderer-side PDF-to-image conversion). Live e2e on dev-pabson-primary tenant `21aea5da-…` school `4209e3d8-…`: real invoice `INV-420-2605-0192.pdf` (15,321B) + real receipt `RCP-420-2605-0008.pdf` (17,034B) downloadable via UI; all 3 branding asset slots (logo / signature / letterhead) upload + render successfully on dev-pabson-primary.
...
| EPIC-C | **C.1.8 — Letterhead background PDF rendering gap** | 🟢 **shipped to prod 2026-05-27** | [PR #211](https://github.com/shoaibrain/edforge/pull/211) (commit `4b2dc50`); `@aibrains/pdf-renderer@0.7.0` live on npm (verified via `npm view`); finance image `sha256:90359d1c…` + identity image `sha256:43a8bf1f…` running on prod-basic; deploy logs `prod-build-application-finance-20260527-083651-9075379.log` + `prod-build-application-identity-20260527-083720-9075379.log` + `prod-ecs-roll-financebasic-…` + `prod-ecs-roll-identitybasic-…` + `prod-ecs-wait-{finance,identity}basic-…` | **The fix:** `<Page>` primitive extended with optional `letterheadBackgroundSrc` prop rendering an absolutely-positioned `<Image fixed>` at full page bounds beneath content (Z-order via DOM declaration; `fixed` flag repeats on multi-page docs). `<InvoicePdf>` + `<ReceiptPdf>` thread `branding.letterheadBackgroundSrc` to Page. **PDF-letterhead guard:** new exported `isPdfLetterheadKey(s3Key)` helper at the finance projection boundary drops `.pdf`-keyed letterheads (M3 phase 2 upload allowlist accepts `application/pdf`, but `@react-pdf/renderer@^3.4.5`'s `<Image>` primitive renders only raster formats). Upload stays valid in S3 for future renderer with PDF-to-image conversion; current render produces no-letterhead PDF rather than crashing inside `renderToBuffer`. Both receipt + invoice renderers share the helper via import. **Tests:** pdf-renderer +4 render-smoke specs (no letterhead, Buffer source, Z-order with BrandedHeader, fixed multi-page) — 98/98 green (was 94). Finance +7 `isPdfLetterheadKey` unit specs — 13/13 green across invoice + receipt renderer suites. **Cross-repo pin sweep:** `@aibrains/pdf-renderer` 0.6.0 → 0.7.0 + consumer pins in `server/application/package.json` + `server/package.json`. **AdminWeb does NOT import pdf-renderer** (verified pre-publish via grep returning empty) → jsdom-sim N/A. **Deploy ladder executed 2026-05-27 PM:** npm publish 0.7.0 → both ECR pushes (finance + identity) in parallel ~2 min each → both `aws ecs update-service --force-new-deployment` accepted → `services-stable` waiters returned for both → image SHA verified on running tasks. **Lessons (L27 + L28):** descriptor types promising `ImageSource` fields MUST have a primitive consumer or the contract is a memory leak in the design; frontend MIME allowlists that diverge from renderer capabilities are a UX trap (M3 phase 2's `application/pdf` for letterhead was operator-intent-driven but renderer-incapable). Projection-layer guard chosen to preserve operator-intent contract for future renderer upgrade. |
...
- **Critical-path next moves (in order, post-C.1.8 ship 2026-05-27 PM):**
  1. **EPIC-C closeout housekeeping** (frontend only, in flight) — Sprint M1.5-FU.7.2-.7.5 receipt-flow UX hardening (~80-120 LOC) + BrandingFileField eager preview UX (~20-30 LOC). Pilot-polish; clears the deferred-work backlog from M2/M3 pivot decisions. ~1 day.
  2. **D.4 BLE Workflow** — research-resolved, dependency-clear, pilot-critical (BLE prep Spring 2027 per master plan §1.2). EPIC-D has no PDF dependency, so this is the natural critical-path move while C.2 backend is queued. 9-ticket sprint locked.
  3. **C.2 Template Editor backend + Sprint M5 frontend** — when prioritized; closes the operator-facing UX loop on PDFs (flip `source:'default'` → `'persisted'` via UI).
  4. **C.3 ReportCard backend + Sprint M7 frontend** — term-end pilot-critical for Saraswati. Mirrors C.1.5 pattern; ~1 day backend + ~half-day frontend.
...
> **Status snapshot 2026-05-27 PM:** C.0 + C.0-followup + C.0-followup-2 (server hotfixes for M3 phase 2) + C.1.1-C.1.6 backend + C.1.7 frontend M2/M3 + **C.1.8 letterhead rendering** all 🟢 shipped to prod. **Frontend EPIC-C closeout** (C.1.6 retire-jspdf + M2 Branding read + M3 phase 1 + M3 phase 2 — total 28 frontend PRs + 3 server PRs #209/#210/#211 between 2026-05-26 and 2026-05-27) shipped. **Two open EPIC-C work items remain before pilot greenlight:** (a) **C.2 backend** — Template Editor write endpoints (`POST/PATCH/POST publish/POST republish/GET versions`); (b) **C.3 backend** — ReportCard render endpoint. Frontend Sprints M5 (C.2 editor UI), M7 (C.3 report-card download), M8 (C.4 batch), M9 (C.5 admit card), M10 (operator UAT) are fully scoped + ready in [`pdf-service-mfe-integration-plan.md`](./pdf-service-mfe-integration-plan.md), each gated on its backend prereq. **EPIC-C pilot-completing housekeeping still open:** Sprint M1.5-FU.7.2-.7.5 receipt-flow UX hardening + BrandingFileField eager-preview UX (frontend only, ~150 LOC) — these were deferred behind M2/M3 and are now safe to resume; tracked in PDF MFE plan §0.6.A.1 + §0.6.A.2.
