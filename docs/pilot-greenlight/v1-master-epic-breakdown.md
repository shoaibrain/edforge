...
> **Updated:** 2026-05-27 — **EPIC-C C.1 + branding (M2 + M3) frontend stack fully shipped to prod end-to-end.** C.1 backend stacked deploy shipped 2026-05-26 (commit `d2cf929`, ~6 min, zero cross-stack export changes). C.1.6 frontend stack (23 PRs across M0/M1/M1.5-FU) shipped 2026-05-26 incrementally to Vercel `edforge.app`. M2 Branding read (PR #88) shipped 2026-05-26. M3 phase 1 Branding write text+colors (PR #89) shipped 2026-05-26. M3 phase 2 Branding asset uploads (FE PR #90 + server hotfixes #209 `requestChecksumCalculation: 'WHEN_REQUIRED'` + #210 PdfAssetsBucket CORS) shipped 2026-05-27. Live e2e on dev-pabson-primary tenant `21aea5da-…` school `4209e3d8-…`: real invoice `INV-420-2605-0192.pdf` (15,321B) + real receipt `RCP-420-2605-0008.pdf` (17,034B) downloadable via UI; all 3 branding asset slots (logo / signature / letterhead) upload successfully and persist. **One pre-existing renderer feature gap surfaced by M3 phase 2 prod test:** `letterheadBackgroundSrc` is declared in `descriptors/types.ts:66` and passed by both finance renderers but no `Page`/`Document` primitive consumes it — letterhead PDF persists in S3 but does NOT render in the output PDF. Tracked as **C.1.8** in the status grid below; ~50-80 LOC + 1 fixture + pdf-renderer 0.7.0 publish + consumer pin sweep + ECR rebuild.
...
| EPIC-C | **C.1.8 — Letterhead background PDF rendering gap (NEW work, pilot-completing)** | 🔲 **not started — pre-existing feature gap surfaced by M3 phase 2 prod test 2026-05-27** | `pdf-service-mfe-integration-plan.md` §0.6.B.1 carries the scope | `letterheadBackgroundSrc?: ImageSource` is declared at [packages/pdf-renderer/src/descriptors/types.ts:66](packages/pdf-renderer/src/descriptors/types.ts#L66) on `BrandingPdfData`. Both finance renderers pass it: [receipt-pdf.renderer.ts:169](server/application/microservices/finance/src/payments/receipt-pdf.renderer.ts#L169) and [invoice-pdf.renderer.ts:141](server/application/microservices/finance/src/invoices/invoice-pdf.renderer.ts#L141). But `grep letterheadBackgroundSrc packages/pdf-renderer/src` returns ONLY the type declaration — no `Page` / `Document` / `BrandedHeader` / descriptor template consumes it. [Page.tsx:38](packages/pdf-renderer/src/primitives/Page.tsx#L38) renders `backgroundColor: DEFAULT_COLORS.background` and nothing else. **Fix:** extend `Page` (or `Document`) to render an absolutely-positioned `<Image>` primitive at full A4 page bounds (595×842pt at 72dpi) behind content when `branding.letterheadBackgroundSrc` is provided. Bumps `@aibrains/pdf-renderer` to 0.7.0 → consumer pin sweep in `server/application/package.json` + `server/package.json` (per `edforge_shared_types_caret_pin`) + ECR rebuild for finance + identity. Snapshot test against PABSON letterhead fixture. AdminWeb does NOT import pdf-renderer (verify before publish) → jsdom-sim N/A. Originally scoped under C.0.3 layout primitives; descriptor promised the field but no primitive consumed it. **Operators can NOT actually use M3 phase 2 letterhead upload until this lands.** ~50-80 LOC + 1 fixture. |
...
- **Sprints with research resolved + ready-to-execute next:**
  ...
  - **C.1.8 letterhead background rendering** — pdf-renderer feature gap; ~50-80 LOC + 1 fixture + 0.7.0 publish + ECR rebuild.
...
- **Critical-path next moves (in order, post-2026-05-27):**
  1. **C.1.8 letterhead rendering** — small, pilot-completing, independent of any other work. Closes the loop on M3 phase 2 letterhead upload actually being meaningful. ~half a day.
  2. **EPIC-C closeout housekeeping** — frontend Sprint M1.5-FU.7.2-.7.5 receipt-flow UX hardening (small, ~80-120 LOC); BrandingFileField eager preview UX (PDF MFE plan §0.6.A.2; ~20-30 LOC). Both deferred behind M2/M3 during pivot decisions; safe to land now that the bigger surfaces are stable.
  3. **D.4 BLE Workflow** — research-resolved, dependency-clear, pilot-critical (BLE prep Spring 2027 per master plan §1.2). EPIC-D has no PDF dependency, so this is the natural critical-path move while C.2 backend is queued.
  4. **C.2 Template Editor backend** — when prioritized; unblocks frontend Sprint M5 immediately.
...
> **Status snapshot 2026-05-27:** C.0 + C.0-followup + C.0-followup-2 (server hotfixes for M3 phase 2) + C.1.1-C.1.6 all 🟢 shipped to prod. **Frontend EPIC-C closeout** (C.1.6 retire-jspdf + M2 Branding read + M3 phase 1 + M3 phase 2 — total 28 frontend PRs + 2 server hotfixes between 2026-05-26 and 2026-05-27) shipped to prod via Vercel. Detailed scope + per-sprint ticket breakdown for the frontend integration lives in [`pdf-service-mfe-integration-plan.md`](./pdf-service-mfe-integration-plan.md) §0 + §1 + §0.6 (remaining-work catalog). **Three open EPIC-C work items remain before pilot greenlight:** (a) **C.1.8** — render `letterheadBackgroundSrc` in pdf-renderer (~50-80 LOC + 0.7.0 publish + ECR rebuild); (b) **C.2 backend** — Template Editor write endpoints; (c) **C.3 backend** — ReportCard render endpoint. Frontend Sprints M5 (C.2 editor UI), M7 (C.3 report-card download), M8 (C.4 batch), M9 (C.5 admit card), M10 (operator UAT) are fully scoped + ready in the PDF MFE integration plan, each gated on its backend prereq.
...
- **C.1.8** — **Render `letterheadBackgroundSrc` in pdf-renderer** (NEW ticket, pre-existing feature gap surfaced by M3 phase 2 prod test 2026-05-27).
  - Files: [`packages/pdf-renderer/src/primitives/Page.tsx`](packages/pdf-renderer/src/primitives/Page.tsx) (or `Document.tsx`) — render an absolutely-positioned `<Image>` primitive at full A4 page bounds (595×842pt at 72dpi) behind content when `branding.letterheadBackgroundSrc` is provided. New snapshot test against PABSON letterhead fixture in `packages/pdf-renderer/src/__tests__/`. Bump `@aibrains/pdf-renderer` to 0.7.0 → npm publish → consumer pin sweep in `server/application/package.json` + `server/package.json` (per [[edforge-shared-types-caret-pin]]) + ECR rebuild for finance + identity (both consume the renderer). AdminWeb does NOT import pdf-renderer (verify pre-publish, per `pdf-service-mfe-integration-plan.md` §0.4 ground truth #9); skip jsdom-sim.
  - Validation: snapshot stable + live download on dev-pabson-primary after a PABSON letterhead is uploaded → resulting invoice/receipt PDF visibly carries the letterhead as page background.
  - AC: All current 93/93 pdf-renderer specs still pass; +1 new letterhead snapshot. Existing PDFs without `letterheadBackgroundSrc` render byte-identically (additive change, opt-in). Operator can finally make M3 phase 2 letterhead upload actually appear in PDFs.
  - Deps: C.0.3 (✅) + C.1.7 (✅).
  - Status: 🔲 not started — pilot-completing follow-up. Originally implied by C.0.3 layout primitives but never made concrete (descriptor type promised the field at [packages/pdf-renderer/src/descriptors/types.ts:66](packages/pdf-renderer/src/descriptors/types.ts#L66); both finance renderers pass it at [receipt-pdf.renderer.ts:169](server/application/microservices/finance/src/payments/receipt-pdf.renderer.ts#L169) and [invoice-pdf.renderer.ts:141](server/application/microservices/finance/src/invoices/invoice-pdf.renderer.ts#L141); but `grep letterheadBackgroundSrc packages/pdf-renderer/src` returns ONLY the type declaration — no primitive or descriptor consumes it). ~50-80 LOC + 1 fixture.
