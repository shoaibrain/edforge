...
- **Critical-path next moves:** `shared-api-routes-stack` split sprint (R41 — shared-infra-stack at 87.7%, hits 90% threshold next sprint); then D.2 + D.3 + C.1 + A.1 (all parallel-eligible after split lands)
