...
> **Updated:** 2026-05-25 (Sprint C.0 — `@aibrains/pdf-renderer` foundation + branding endpoints — closed 🟢 with 2 documented HIGH gaps for a C.0-followup; next move = **C.0-followup → C.1.1 InvoicePdf component**).
...
| EPIC-C | C.0 PDF Foundation + Branding | 🟡 **shipped 2026-05-25 with 2 HIGH followups** | PRs [#181](https://github.com/shoaibrain/edforge/pull/181) (design), [#182](https://github.com/shoaibrain/edforge/pull/182)–[#185](https://github.com/shoaibrain/edforge/pull/185) (pdf-renderer 0.1→0.4), [#187](https://github.com/shoaibrain/edforge/pull/187) (shared-types 0.61.0), [#188](https://github.com/shoaibrain/edforge/pull/188) (S3 buckets), [#189](https://github.com/shoaibrain/edforge/pull/189) (branding endpoints) + hotfixes [#190](https://github.com/shoaibrain/edforge/pull/190) (JwtAuthGuard) + [#191](https://github.com/shoaibrain/edforge/pull/191) (TVM cred case) + docs [#192](https://github.com/shoaibrain/edforge/pull/192); deploy logs `prod-build-application-identity-…-2ce4a26` + `analytics-prod-…-e533ac9` + `prod-smoke-c-0-7-branding-…-2ce4a26` | All 7 tickets live. `@aibrains/pdf-renderer@0.4.0` + `@aibrains/shared-types@0.61.0` live on npm. PDF S3 buckets `edforge-pdfs-*` (tag-lifecycle 7d, TagFilter `lifecycle=pdf-jobs`) + `edforge-pdf-assets-*` (versioned) live in prod ap-south-1. Identity image `sha256:f680c1cd…` on task def revision 5. ABAC role: `s3:Get/PutObject` on `tenants/${aws:PrincipalTag/tenant}/*` (no DeleteObject per CodeRabbit). 5/5 smoke green on dev-pabson-primary. **2 HIGH followups identified by 2026-05-25 audit** (filed as Sprint C.0-followup, see §5): (a) Permission key shipped as `@RequireGlobalRole('TenantAdmin')` instead of spec'd `branding:configure` (TenantAdmin + Principal school-scoped) — Principal users locked out; (b) `GET /schools/:id/branding` returns raw S3 keys instead of spec'd signed GET URLs — frontend must presign per-asset. **MEDIUM gap:** BrandingService has no unit specs (smoke-only). Both hotfix-class typos retros captured in memory `project_sprint_c0_7_shipped_prod`. |
| EPIC-C | C.0-followup (audit-driven 2026-05-25) | 🔲 next | — | Closes 2 HIGH gaps from C.0.7 audit before C.1 kicks off: (a) add `branding:configure` permission with Principal school-scoped role support — extend `BrandingController` to use `@RequirePermission` or composite role decorator; (b) update `BrandingService.getBranding()` to return signed GET URLs (`logoUrl`, `principalSignatureUrl`, `letterheadBackgroundUrl`) alongside raw keys, via `S3PresignerService.presignGet()` (already exists, currently unused); (c) add `branding.service.spec.ts` with mocked TVM + mocked S3 client covering presign + tenant-scoping + MIME/size validation. Then deploy ladder identical to C.0.7. ETA: 1 day. |
| EPIC-C | C.1 – C.5 PDF Generation Service | ⏳ **next after C.0-followup** (revised plan 2026-05-24) | sprint plan: [`c-epic-pdf-generation-design.md`](./c-epic-pdf-generation-design.md) | **Plan amended 2026-05-24:** renumbered to C.0–C.5 (24 atomic tickets across 6 sprints); renderer choice swapped from Puppeteer+Handlebars to `@aibrains/pdf-renderer` (built on `@react-pdf/renderer`); template editor moved from AdminWeb to tenant-facing Shell-Settings; lazy-seed pattern adopted (no backfill needed). C.1+C.2 ship independently of EPIC-D. R23 (Lambda cold-start) closed by renderer change. **C.1 sub-sequence:** C.1.1 InvoicePdf component (publish 0.5.0) → C.1.2 ReceiptPdf (0.6.0) → C.1.3 PdfTemplatesService read-only → C.1.4 IdentityClient.getCurrentTemplate cache → C.1.5 finance GET /invoices/:id/pdf → C.1.6 finance GET /payments/:id/receipt/pdf + retire jspdf. |
...
- **Sprints partially shipped (🟡, with documented gaps):** 3 — 0.1 (0.1.3 reclassified to E.1.5 per §17.6), D.1 (R38 mapper serializer followup), **C.0** (2 HIGH followups identified by 2026-05-25 audit: permission model excludes Principal role; GET returns raw S3 keys instead of signed URLs — see C.0-followup ticket in §5)
- **Sprints with research resolved + ready-to-execute:** D.4 (all prereqs ✅: D.1 + D.2 + D.3 + A.2 + E.0.2); EPIC-C C.1–C.5 (revised plan landed 2026-05-24 — see [`c-epic-pdf-generation-design.md`](./c-epic-pdf-generation-design.md))
- **Critical-path next moves (per 2026-05-25 audit):** ⏳ **EPIC-C C.0-followup** (close the 2 HIGH gaps before C.1 inherits them) → **Sprint C.1 (Invoice + Receipt PDF MVP)**. PDF generation prioritized over D.4 BLE Workflow because (a) all V1 PABSON pilot doc types (Invoice, Receipt, Report Card) ship in C.0–C.3 without EPIC-D dependency; (b) Saraswati Term-1 operator value compounds the moment Invoice/Receipt PDFs land; (c) D.4 admit-card integration interleaves with EPIC-D at C.5 anyway.
...
**Status:** 🟡 **shipped 2026-05-25 with 2 HIGH followups** (see new "C.0-followup" sub-sprint below). All 7 tickets live; foundation library + branding-write path proven end-to-end via 5/5 prod smoke on dev-pabson-primary. **The `<InvoicePdf>` component named in the original "Demo (library)" line is NOT shipped by C.0** — that's C.1.1.
...
**Status:** 🔲 next. **Blocks no C.1 ticket directly** but the gaps would compound into C.2 editor RBAC if not closed first. Estimated 1 day.
...
**Deploy ladder for C.0-followup:** identical to C.0.7 — ECR push identity → `tenant-template-stack-basic` (transitively redeploys `shared-infra-stack` for the OpenAPI asset bump if any) → smoke. Probably one PR per ticket, then one consolidated ECR push + ECS roll.
