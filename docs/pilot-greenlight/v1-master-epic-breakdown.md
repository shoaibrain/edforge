...
| EPIC-A | A.4 Result Subsystem | 🟡 Phase 1+2 shipped 2026-05-23 (incident + recovery); Phase 3+4 pending | PRs [#161](https://github.com/shoaibrain/edforge/pull/161) Phase 1 shared-types schemas + [#162](https://github.com/shoaibrain/edforge/pull/162) Phase 2 academics service + [#163](https://github.com/shoaibrain/edforge/pull/163) hotfix; sprint plan `docs/pilot-greenlight/a4-sprint-plan.md`; foundation audit `a4-foundation-readiness-audit.md`; incident memory `project_a4_phase2_incident` | A.4.1–A.4.6 live on prod after recovery. shared-types 0.58.0 published; academics image `sha256:2c9fd8b8…` running on `prod-basic/academicsbasic` post-hotfix; shared-infra-stack redeployed with 5 new `/academics/result-cards/*` API GW paths; smoke `GET /academics/result-cards` returns 200+`[]`. **A.4 Phase 2 caused a ~4h academics outage 06:15-12:19 UTC 2026-05-23 from missing IdentityClientService provider** (§17.9 L9; R43); recovered via image-tag rollback to A.3 :latest; hotfix #163 added the provider + new `academics/__tests__/module-wiring.spec.ts` (43 assertions). R42 closed by aggregation studentId resolution. Phase 3 (result-batch Lambda + EventBridge + DLQ + alarms) + Phase 4 (smoke) pending. |
...
- **Critical-path next moves:** A.4 Phase 3 (result-batch Lambda + EventBridge); D.2 + D.3 + C.1 + A.1 (all parallel-eligible)
