...
> **Updated:** 2026-05-25 PM (Sprint C.0-followup + Sprint C.1.1 both shipped to prod 🟢. C.0-followup closed the 2 HIGH gaps from the 2026-05-25 morning audit; C.1.1 shipped `<InvoicePdf>` + invoice descriptor as `@aibrains/pdf-renderer@0.5.0` on npm. Next move = **C.1.2 ReceiptPdf component**).
...
| EPIC-C | C.1.2 – C.5 PDF Generation Service | ⏳ **next** (revised plan 2026-05-24) | sprint plan: [`c-epic-pdf-generation-design.md`](./c-epic-pdf-generation-design.md) | **C.1 sub-sequence (C.1.1 done):** C.1.2 ReceiptPdf (publish 0.6.0) → C.1.3 PdfTemplatesService read-only (identity) → C.1.4 IdentityClient.getCurrentTemplate LRU cache helper → C.1.5 finance GET /invoices/:id/pdf endpoint → C.1.6 finance GET /payments/:id/receipt/pdf + retire jspdf in PaymentReceipt.tsx. Then C.2 (Shell editor) + C.3 (Report Card) + C.4 (Batch Lambda) + C.5 (Admit Card data-shape). C.1+C.2 ship independently of EPIC-D. |
...
- **Sprints partially shipped (🟡, with documented gaps):** 2 — 0.1 (0.1.3 reclassified to E.1.5 per §17.6), D.1 (R38 mapper serializer followup). **C.0 elevated to 🟢** as of 2026-05-25 PM (C.0-followup closed the 2 HIGH gaps end-to-end).
- **Newly-shipped 2026-05-25 PM (🟢):** C.0-followup (PR #195) closes the 2 HIGH gaps + 1 MEDIUM test debt; C.1.1 (PR #196) ships `<InvoicePdf>` component via `@aibrains/pdf-renderer@0.5.0` on npm. Total 8 PRs landed on 2026-05-25 across C.0 audit + C.0-followup + C.1.1 — the most productive single day in the EPIC-C cycle.
- **Sprints with research resolved + ready-to-execute:** D.4 (all prereqs ✅: D.1 + D.2 + D.3 + A.2 + E.0.2); EPIC-C C.1.2–C.5 (revised plan landed 2026-05-24 — see [`c-epic-pdf-generation-design.md`](./c-epic-pdf-generation-design.md))
- **Critical-path next moves:** ⏳ **Sprint C.1.2 ReceiptPdf** (composition exercise on the C.1.1 primitives; publish 0.6.0) → C.1.3 PdfTemplatesService → C.1.4 IdentityClient cache → C.1.5 finance Invoice render endpoint (first user-visible PDF in prod). PDF generation prioritized over D.4 BLE Workflow per CEO 2026-05-24 call.
