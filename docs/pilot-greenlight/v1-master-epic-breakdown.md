...
> **Updated:** 2026-05-25 evening — **C.1 phase code-complete on `main`**. All six C.1 PRs (#196 InvoicePdf · #198 ReceiptPdf · #199 PdfTemplatesService · #200 IdentityClient cache · #201 finance invoice PDF · #202 finance receipt PDF) merged to `main` 2026-05-25. **Stacked AWS deploy pending operator authorization:** shared-infra-stack CDK (2 new API GW routes: invoice + receipt PDF) → identity ECS roll (C.1.3 + C.1.4) → finance ECS roll (C.1.5 + C.1.6) → live smoke on dev-pabson-primary. Frontend C.1.6 (retire `jspdf+html2canvas` in `PaymentReceipt.tsx`) is the **next concrete code work** — separate `edforge-saas-frontend` PR; can be authored ahead of backend deploy + merged after backend smoke green.
...
| EPIC-C | C.1.3 PdfTemplatesService (identity) | 🟢 **merged 2026-05-25** (deploy stacked) | [PR #199](https://github.com/shoaibrain/edforge/pull/199) + CodeRabbit fix `9369244` | Read-only `GET /schools/:id/pdf-templates/:docType/current` on identity; lazy-default returns `descriptor.defaults()` on persisted miss (NO DDB write). Adds `PDF_TEMPLATE_CURRENT` + `PDF_TEMPLATE_VERSION` entity types + EntityKeyBuilder helpers + `pdf-templates:view` permission (granted to Principal). Lockstep dep pin `@aibrains/pdf-renderer@^0.6.0` on server/application + server pkg. New `server/application/webpack.config.js` externalizes `@react-pdf/*` + `yoga-layout` (yoga ships raw `.ts` in node_modules → webpack parse error). CodeRabbit fix: hardened bearer-token extraction (`/^\s*Bearer\s+/i`) + corrected `@throws` JSDoc. 111/111 specs. **shared-infra-stack CDK redeploy required** for the new API GW route. |
| EPIC-C | C.1.4 IdentityClient.getCurrentTemplate + 60s LRU | 🟢 **merged 2026-05-25** (libs-only, no deploy) | [PR #200](https://github.com/shoaibrain/edforge/pull/200) + CodeRabbit fix `1ad0d33` | Lockstep helper on academics + finance IdentityClient. Per-process LRU cache (60s TTL, 100-entry cap), 5xx fallback → `descriptor.defaults(fallbackArchetype)`, 4xx propagates. CodeRabbit fixes: single-flight dedup via `pendingPdfTemplateRequests` Map (stampede guard), LRU refresh-order fix (`delete+set` for MRU), `encodeURIComponent` on path segments. 27/27 specs. Lesson L23: use `.then(cleanup, cleanup)` not `.finally()` on unawaited cleanup chains. **No deploy in isolation** — helper unreachable until C.1.5 consumes it. |
| EPIC-C | C.1.5 finance GET /invoices/:id/pdf — **first user-visible PDF in prod** | 🟢 **merged 2026-05-25** (deploy stacked) | [PR #201](https://github.com/shoaibrain/edforge/pull/201) + CodeRabbit fix `758928a` | THE PR. New `invoice-pdf.renderer.ts` pure projection + `InvoicesService.getPdf` + `@Get(':id/pdf')` with `billing:view` + ownership + binary response (Content-Type: application/pdf, inline disposition, Cache-Control: private,no-store). Branding fetch error degrades to `null` branding; template 5xx falls through to C.1.4 PABSON default. Audit = CloudWatch structured `pdf_generated` log (NOT DDB — cross-service writes to identity's AuditLog not in V1 architecture). Explicit `react@^18.3.1` + `@react-pdf/renderer@^3.4.5` deps on server/application/package.json (finance ACTUALLY renders at runtime; identity only uses descriptor.defaults() JSON). CodeRabbit fix: defensive `resolvePrimaryLocale(unknown)` helper guards against malformed `labelLanguages` at JSON boundary. PDFs self-contained post-render (react-pdf inlines image bytes; 10-min S3 logo URL TTL irrelevant). 35/35 specs across 4 suites. **shared-infra-stack CDK redeploy + finance ECR push + ECS roll required.** B0.1: controller + service both fetch invoice (2× DDB GetItem; same partition, ~5ms). |
| EPIC-C | C.1.6 finance GET /payments/:id/receipt/pdf (backend) | 🟢 **merged 2026-05-25 — closes C.1 phase backend-side** | [PR #202](https://github.com/shoaibrain/edforge/pull/202) + CodeRabbit fix (schoolId query param) | Mirror of C.1.5 for RECEIPT docType. New `receipt-pdf.renderer.ts` (Payment + Invoice → ReceiptDocumentData projection, with gateway display name table) + `PaymentsService.getReceiptPdf` (loads Payment entity directly; 400 if status !== 'completed'; parallel branding + template + invoice fetch) + `@Get('payments/:paymentId/receipt/pdf')`. Sparse-payment fallbacks: null `receiptNumber` → synthesized `RCP-<8-char>`; null `paidBy` → `invoice.studentName`. 28/28 specs (13 new C.1.6 + 15 regression). **shared-infra-stack CDK redeploy + finance ECS roll required** (route bundles with C.1.5). |
| EPIC-C | C.1.6 frontend (`edforge-saas-frontend`) — retire jspdf | ⏳ **next** (separate PR; auth ahead of backend deploy possible) | TBD on `edforge-saas-frontend` | (a) NEW `useDownloadReceiptPdf(paymentId)` hook in `packages/finance-services/src/hooks/usePayments.ts` mirroring `useExportInvoicesCsv` blob-anchor pattern. (b) Replace `handleDownloadPdf` (jspdf+html2canvas at lines 29-53) in `apps/shell/src/components/payments/PaymentReceipt.tsx`. (c) Invoice page `apps/finance/src/routes/billing/invoices/$invoiceId.tsx` line 98 — add `[Download PDF]` button next to existing `[Print]`. (d) Drop `jspdf` + `html2canvas` from `apps/shell/package.json` once C.5 lands and no other consumer. Print button stays through C.5. |
| EPIC-C | C.2 Template Editor (Shell) | 🔲 not started | sprint plan: [`c-epic-pdf-generation-design.md`](./c-epic-pdf-generation-design.md) §6 | Tenant-facing Shell editor at `/settings/pdf-templates`. TenantAdmin manages all doc-type templates from one place; reads `descriptor.configurableFields[]` per registered doc type and renders form sections automatically. Writes via `PATCH /schools/:id/pdf-templates/:docType` (NEW endpoint — adds the operator save path that flips C.1.3's lazy-default to a persisted-source response). Bumps `pdf-templates` permission to `view + configure`. New permission grant for Principal. |
| EPIC-C | C.3 ReportCard render (academics) | 🔲 not started — gated on C.1.6 frontend + C.2 readiness | — | `<ReportCardPdf>` document + report-card descriptor + academics `GET /result-cards/:id/pdf`. Mirrors C.1.5 pattern with REPORT_CARD docType. Hard deps: A.4 (✅) + C.0.4 (✅) + C.1.4 (✅). |
| EPIC-C | C.4 Batch Lambda (`PdfJobRequested` consumer) | 🔲 not started | — | Future async batch path for end-of-term report-card runs. Consumes `pdf.job.requested` from EventBridge, renders via the same `@aibrains/pdf-renderer` library, uploads to `edforge-pdfs-*` S3, emits `pdf.job.completed`. Uses ScriptJob-style Lambda not ECS (concurrency ramps to 1000 in ap-south-1). |
| EPIC-C | C.5 AdmitCard data-shape | 🔲 not started | — | `<AdmitCardPdf>` document + admit-card descriptor — once data-shape is locked, ship in mirror of C.1.5. Per master plan, locks the operator-facing surface for the Saraswati municipal-exam pilot use case. |
...
- **Sprints fully shipped + validated to prod (🟢):** 9 — 0.4, A.2, A.3, A.4, **D.2**, **D.3**, E.0, E.1, **C.0** (C.0-followup [PR #195](https://github.com/shoaibrain/edforge/pull/195) closed the 2 HIGH audit gaps end-to-end 2026-05-25 PM).
- **🟢 Merged-to-`main`, AWS deploy pending operator authorization (1 stacked deploy):** **the entire C.1 phase (C.1.1–C.1.6 backend)**. 5 PRs (#196 InvoicePdf · #198 ReceiptPdf · #199 PdfTemplatesService · #200 IdentityClient cache · #201 finance invoice PDF · #202 finance receipt PDF). C.1.1 + C.1.2 are npm-only (no AWS deploy needed; both already live on npm at @aibrains/pdf-renderer@0.5.0 + 0.6.0). C.1.3 + C.1.4 + C.1.5 + C.1.6 need ONE coordinated deploy: shared-infra-stack CDK redeploy (2 new API GW routes for invoice + receipt PDF) → identity ECS roll (C.1.3 + C.1.4) → finance ECS roll (C.1.5 + C.1.6) → live smoke on dev-pabson-primary.
...
- **Newly-merged 2026-05-25 (six C.1 PRs in one day):** #196 + #198 + #199 + #200 + #201 + #202. With C.0 + C.0-followup + the C.1 stack, 2026-05-25 closed **12 PRs** in the EPIC-C cycle — the most productive single day in the V1 sprint history. All six C.1 PRs received CodeRabbit findings + fixes in-PR before merge (8 total CodeRabbit catches across the C.1 series; all merged with regression specs covering the fix).
- **Sprints with research resolved + ready-to-execute:** D.4 BLE Workflow (all prereqs ✅: D.1 + D.2 + D.3 + A.2 + E.0.2 — 9-ticket sprint locked); EPIC-C C.2 (Shell editor), C.3 (ReportCard), C.4 (Batch Lambda), C.5 (AdmitCard) — all gated on C.1 backend deploy + C.1.6 frontend.
- **Critical-path next moves (in order):**
  1. **Coordinated C.1 backend deploy** (operator-authorized, ~30-45 min): shared-infra-stack CDK → identity ECS roll → finance ECS roll → live smoke on dev-pabson-primary covering `GET /invoices/:id/pdf` + `GET /payments/:id/receipt/pdf` with Devanagari + BS dates + NPR figures rendered correctly.
  2. **C.1.6 frontend PR** on `edforge-saas-frontend` (separate repo): `useDownloadReceiptPdf` hook + replace jspdf+html2canvas in `PaymentReceipt.tsx`. Can be authored ahead of step 1 + merged after step 1 smoke green.
  3. **C.2 Template Editor (Shell)** OR **D.4 BLE Workflow** — both are research-✅ + dependency-✅. CEO call 2026-05-24 prioritized PDF generation over D.4; with C.1 closed, C.2 is the natural continuation that closes the operator-facing UX loop on PDFs. D.4 can parallelize since EPIC-D has no PDF dependency yet.
