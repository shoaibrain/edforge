...
- **C.0.6** — Tenant PDF buckets via CDK.
  ...
  - Validation: `cdk synth` clean; `cdk deploy analytics-stack` via `scripts/deploy.sh analytics-stack prod`; raw deploy log kept private, sanitized deploy note added.
...
- [ ] Private deploy evidence summarized in `docs/deploys/INDEX.md` for prod-touching actions
