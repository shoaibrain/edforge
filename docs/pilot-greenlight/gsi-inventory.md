...
The fourth FB row shape, `AGREEMENT_TERM_LOCK#{schoolId}#{studentId}#{agreementChainId}` (BH-1.1 / epic §3.6 R11), also populates **no GSI**. It is written ATOMICALLY with an AGREEMENT-priced invoice in one `TransactWriteItems` (`{Put: lock, ConditionExpression: attribute_not_exists(entityKey)}` + `{Put: invoice}`), making the per-term duplicate-billing guard (`assertNoExistingAgreementInvoice`) atomic under CONCURRENT generation — two generations for the same (school, student, chain) can no longer both pass the read-then-put scan and double-bill (the loser's lock put fails its condition → the whole transact rolls back → 409 `AGREEMENT_ACTIVE`, no invoice written). Keyed on the version CHAIN (`versionParentId || agreementId`) so FB-3.6 re-versioning can't re-bill a locked term. Auto-expires via the shared `agreementLockTtl` (`ttl` = effectiveTo + 30-day grace); the read-time guard remains the friendly fast-path. Standard (no-agreement) invoices keep the bare `putItem` (byte-identical golden output). Key builder: `EntityKeyBuilder.agreementTermLock` + `createAgreementTermLockEntity`; pinned by `billing-agreement.entity.spec.ts`.
