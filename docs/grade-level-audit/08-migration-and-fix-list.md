...
status: REVISED 2026-05-08 (2nd pass) — runtime tests rebutted F-CURRICULUM-1, escalated F-LEGACY-1, downgraded F-CONFIG-1
date: 2026-05-08
...
| **F-IEMIS-2** | Cross-validate row's `IEMIS Code` against the controller's `schoolId` (reject mismatches with finding) | MEDIUM (data-integrity hazard for multi-school tenants) | Academics [iemis-transform.ts:132-138](../../server/application/microservices/academics/src/students/iemis-transform.ts#L132) | None | Neutral | none | Build a test xlsx where `IEMIS Code` belongs to School A but post to controller with `schoolId=B`. Observe rows land in B with no warning. | **S** |
...
2. **F-IEMIS-1 + F-IEMIS-2** (MEDIUM, S each) — bundle into one PR. Janitor cron for orphan-running-job + cross-school IEMIS Code validation. Protects next pilot's import.
...
The next pilot (when scheduled) raises F-IEMIS-1 and F-IEMIS-2 to "must have." Time the FIX sprint to land before next-pilot kickoff.
