...
| S5 | **`BULK_PDF_CONCURRENCY`: default 40, hard max 40, per-task scope (process-wide singleton p-limit).** | **Operator override (2026-06-30, after MVP.1 right-sizing landed):** the original operator directive (max 40) is restored as both default and hard max. Critic had argued for 4/16 based on the prior 0.25 vCPU profile; after MVP.1's 4× CPU + 4× memory bump, the operator accepts the 40-way concurrency budget. **Per-task scope is preserved** — process-wide singleton p-limit, NOT per-job (prevents "two concurrent bulk jobs each get N-way = 2N total"). F.3 worker MUST still apply `Math.min(40, Math.max(1, parsed))` as the floor/ceiling guard against malformed env values. Sprint I.7 load test will validate the 40-way budget against the 256 MB RSS ceiling; the env var is tunable without redeploy if RSS climbs. |
...
- **Description:** Add `BULK_PDF_CONCURRENCY` to the finance container environment in `service-info.txt` with default value `"40"`, and to the `ContainerInfo` TS interface. Worker code in F.3 will read it via `parseInt(process.env.BULK_PDF_CONCURRENCY ?? '40', 10)` with the hard ceiling of 40 from §5d S5 (operator-overridden from the critic's 16).
- **Acceptance:**
  - `service-info.txt` finance env has `BULK_PDF_CONCURRENCY: "40"`
  - [server/lib/interfaces/container-info.ts](server/lib/interfaces/container-info.ts) has the new optional slot
  - F.3 ticket's worker code applies `Math.min(40, Math.max(1, parsed))` floor/ceiling — process-wide singleton p-limit, not per-job
