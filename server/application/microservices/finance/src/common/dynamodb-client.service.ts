 ...
 * 
 * DynamoDB Client Service - Shared service for all enrollment modules
 * 
 * ARCHITECTURE:
 * - Single DynamoDB table: school-table-{tier}
 * - Tenant isolation via partition key (tenantId)
 * - Supports all GSIs (GSI1-GSI12)
 */
...
export class DynamoDBClientService {
  ...
  constructor() {
    // Detect local vs AWS environment
    const dynamodbEndpoint = process.env.DYNAMODB_ENDPOINT;
    const isLocal = dynamodbEndpoint && dynamodbEndpoint.includes('localhost');
    ...
    // Finance service uses separate table for PCI compliance
    this.tableName = process.env.TABLE_NAME || process.env.FINANCE_TABLE_NAME || 'finance-table-v2-basic';
    
    const environment = isLocal ? 'LOCAL' : 'AWS';
    this.logger.log(`DynamoDB client initialized [${environment}] for table: ${this.tableName}`);
    
    if (isLocal) {
      this.logger.log(`Using local DynamoDB endpoint: ${dynamodbEndpoint}`);
    }
  }
  ...
  async queryGSI(
    ...
    try {
      ...
      const queryParams: any = {
        TableName: this.tableName,
        IndexName: indexName,
        KeyConditionExpression: `gsi${gsiNumber}pk = :pk`,
      ...
      if (sortKey) {
        if (sortKeyCondition === 'begins_with') {
          queryParams.KeyConditionExpression += ` AND begins_with(gsi${gsiNumber}sk, :sk)`;
          queryParams.ExpressionAttributeValues[':sk'] = sortKey;
        } else if (sortKeyCondition === 'between') {
          queryParams.KeyConditionExpression += ` AND gsi${gsiNumber}sk BETWEEN :sk_start AND :sk_end`;
        } else {
          queryParams.KeyConditionExpression += ` AND gsi${gsiNumber}sk = :sk`;
          queryParams.ExpressionAttributeValues[':sk'] = sortKey;
        }
      }
  ...
  async query(
    ...
    try {
      ...
      if (sortKeyPrefix) {
        queryParams.KeyConditionExpression += ' AND begins_with(entityKey, :prefix)';
        queryParams.ExpressionAttributeValues[':prefix'] = sortKeyPrefix;
      }
      ...
      this.logger.debug(`Query executed: ${result.Items?.length || 0} items returned`);
      return result.Items || [];
    } catch (error) {
      this.handleDynamoError('query', error);
      return [];
    }
  }
  ...
  async checkTableHealth(): Promise<boolean> {
    try {
      const { DynamoDBClient: RawDynamoDBClient } = await import('@aws-sdk/client-dynamodb');
      const { DescribeTableCommand } = await import('@aws-sdk/client-dynamodb');
      
      const dynamodbEndpoint = process.env.DYNAMODB_ENDPOINT;
      const isLocal = dynamodbEndpoint && dynamodbEndpoint.includes('localhost');
      
      const rawClient = new RawDynamoDBClient({
        region: process.env.AWS_REGION || 'us-east-1',
        ...(isLocal && {
          endpoint: dynamodbEndpoint,
          credentials: {
      ...
      await rawClient.send(new DescribeTableCommand({ TableName: this.tableName }));
      this.logger.debug(`Table health check passed: ${this.tableName}`);
      return true;
    } catch (error: any) {
      this.logger.error(`Table health check failed: ${error.message}`);
      return false;
    }
  }

  /**
   * Get DynamoDB client (for advanced operations)
   */
