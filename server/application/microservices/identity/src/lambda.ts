...
/**
 * Lambda entry for the identity service (cost-redesign C1.3).
 *
...
export async function buildHandler(): Promise<ProxyHandler> {
  ...
  const app = await NestFactory.create(IdentityModule, new ExpressAdapter(expressApp), {
    logger: new StructuredLogger('identity-service'),
  });
