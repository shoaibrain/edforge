...
export class SchoolAttendancDerivationService {
  ...
  async deriveSchoolAttendance(
    ...
    // Attendance realignment: derivation always runs. A "homeroom" is no longer a
    // stored entity, so there is no authoritative daily roll-call to defer to —
    // the school-day record is always derived from section attendance. The
    // policy-aware rule (daily_presence "best-status-wins" vs per_section_granular
    // threshold) is applied in the aggregator rework; today this stays
    // worst-status-wins, which neither policy is deployed against yet.
